...
| `defenseclaw setup credential-protection [--disable]` | Install, repair, enable, or disable the local s-gw broker. Disabling preserves `~/.s-gw`. See [Credential protection](/docs/setup/credential-protection). |
| `defenseclaw credential-protection status\|open\|launch` | Show safe broker metadata, request the authenticated approval console, or launch a supported agent through s-gw guard mode. Launch uses a minimal environment unless `--inherit-environment` is explicitly set. |
